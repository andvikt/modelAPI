...
def nans(data: DataFrame):
    return data.isna().sum() / data.shape[0] + 0.000001
...
def mean(data: DataFrame):
    return data.select_dtypes('number').mean() + 0.000001
...
def quantiles(data: DataFrame):
    ...
    return data.select_dtypes('number').apply(quant, result_type='expand').T.stack()
